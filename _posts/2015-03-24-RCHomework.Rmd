...
__Oil prices sank starting in December__, with its lowest level in five years. This became the first blow to the Russian economy.
...
The Russian Central Bank saw __foreign exchange reserves fade away with levels even lower than the 2009 economic crisis.__ 
```{r fig.width=12, fig.height=4,echo=FALSE}
...
ggplot(forexreserves, aes(x = Date, y = Last.Price)) +
  geom_line(aes(group=1), colour="#E69F00", size = 1) + 
...
__Russian inflation reached its highest peak__ for the past half-decade indicating turmoil.
...
A currency crisis was imminent. **The ruble fell to its lowest value** against the US dollar.
...
Russia's main index MICEX seemed to be flat; __however, it was down when priced in US dollars.__
...
g2 <- ggplot(data = micex, aes(x = Date, y = USD)) +xlab(NULL)+
  geom_bar(stat = "identity", position = "identity", fill="#F0E442") +
    ggtitle("MICEX (USD)") +
    theme(axis.title.x = element_blank(), axis.title.y =element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank(), plot.margin = unit(c(1,0,1,-1), "mm"),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white', colour = 'black'), text = element_text(size=10)) +
  coord_flip()
